...
const BasePage = function (customAudio = null) {
  ...
  this.findByText = async function (text) {
    await this.driver.wait(
      until.elementLocated(By.id(text)),
      15000,
      "Looking for element"
    );
    return await this.driver.findElement(By.id(id));
  ...
  this.signin = async function () {
    ...
    let input = await this.findById("login_field");
  ...
  this.createNewRepo = async function () {
    let inputSpan = await this.findByClassName(
      "TextInputWrapper__TextInputBaseWrapper-sc9mqhpbi-0"
    );
    console.log('create repo done');
    console.log(inputSpan);
    await inputSpan.click();
  };

  this.pressCreateButton = async function () {
    let buttons = await this.findByClassName(
      "types__StyledButton-sc-ws60qy-0 dWuCGR"
    );
    await buttons.click();
  };
